			...
			@foreach ($patients as $patient)
				<tbody>
					<tr>
						...
						<td scope="col" class="collapse">
							{{date('d/m/Y', strtotime($patient->birth))}}
						</td>
						<td scope="col" class="collapse">
							{{
								$patient->cellphone ? "(" . $patient->dddc . ") " 
								. $patient->cellphone : "(" . $patient->ddd . ") " 
								. $patient->phone
							}}
						</td>
						<td class="d-flex flex-row">
							<nav>
								<a class="nav-link dropdown-toggle btn btn-success btn-sm" 
									data-toggle="dropdown" href="#" role="button" aria-haspopup="true" 
									aria-expanded="false">
								</a>
								<div class="dropdown-menu">
									<a class="dropdown-item text-success" 
										href="{{ route('patient.show', ['patient' => $patient->patient_id]) }}">
										Exibir
									</a>
